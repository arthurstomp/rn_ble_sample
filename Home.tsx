...
const App = ({navigation}: Props) => {
  ...
  const handleUpdateValueForCharacteristic = (
    _data: BleManagerDidUpdateValueForCharacteristicEvent,
  ) => {
    // console.debug(
  ...
  const poc = async (peripheral: Peripheral) => {
    ...
    if (targetService && targetChar) {
      ...
      const writenBytes = await BleManager.write(
        peripheral.id,
        targetService.uuid,
        targetChar.characteristic,
        Buffer.from('Ping').toJSON().data,
      );
      console.log('WRITEN BYTES', writenBytes);
      // console.log('WRITEN BYTES string', writenBytes.toString('utf-8'));
    }
  };

  const connectPeripheral = async (peripheral: Peripheral) => {
    navigation.navigate('Device', {peripheral});
  };
